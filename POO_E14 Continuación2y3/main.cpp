...
public:
    Login(QWidget *parent = nullptr) : QWidget(parent), tempNum(new QLabel("0")), imagenFondoDownloaded(false), failedAttempts(0) {
        ...
        obtenerTemperatura(); // Llamar a la función para obtener la temperatura
    }

    void setBackgroundImage(const QString &imageUrl) {
        obtenerImagenFondo(imageUrl); // Llamar a la función para obtener la imagen de fondo con la URL proporcionada
    }
...
private:
    ...
    QLabel *tempNum; // Declarar tempNum como un miembro de la clase
    ...
    void verificarClave() {
        if (lineEditClave->text() == "1111") {
            ...
            failedAttempts++;
            if (failedAttempts >= 3) {
                lineEditClave->setEnabled(false);
                lockoutTimer.setSingleShot(true);
                connect(&lockoutTimer, &QTimer::timeout, this, &Login::resetFailedAttempts);
                lockoutTimer.start(300000); // Bloquear durante 5 minutos (300,000 ms)
            }
        }
    }

    void resetFailedAttempts() {
        lineEditClave->setEnabled(true);
        failedAttempts = 0;
    }
